pipeline {
    ...
    stages {
        stage('Download ISO') {
            steps {
                script {
                    copyArtifacts projectName: 'build-vyos-iso-equuleus', filter: 'vyos-*-iso-amd64.iso', target: 'iso/'
                }
            }
        }
            ...
            steps {
                script {
                    sh '''
                        ...
                        sed -i -E "s|cloud_init\\s*=\\s*\\"debian\\"|cloud_init = \\"comment\\"|g" local-vyos-1.3.pkrvars.hcl
